...
/**
 * 전사 공지사항 게시글 정보를 담는 Value Object 클래스.
 * AbstractBoardVO로부터 공통 필드(id, title, content, authorId, authorName, createdAt, views)를 상속.
 * NoticeMapper.java, NoticeService.java에서 사용되며,
 * 관리자(UserVO.java의 isAdmin=1)만 작성 가능.
 * 첨부 파일 목록은 ArchiveFileVO.java 리스트로 관리됨.
 * 팀별 공지는 TeamNoticeVO.java 참조.
 *
 * [Builder Pattern] NoticeVO.builder().title("...").isRequired(1).build()
 */
public class NoticeVO extends AbstractBoardVO {
    ...
    public static class Builder extends AbstractBoardVO.Builder<NoticeVO, Builder> {
        private Integer commentCount;
        private Integer isRequired;
        private String tags;
        private List<ArchiveFileVO> files;
