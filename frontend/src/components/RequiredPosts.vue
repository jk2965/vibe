      ...
      <table class="board-table">
        ...
        <tbody>
          ...
          <tr v-if="flatList.length === 0">
            <td colspan="6" class="empty-cell">등록된 필독 게시글이 없습니다.</td>
          </tr>
          <!-- 현재 페이지 필독 게시글 목록 -->
          <tr
...
// 필독 게시글을 전체 게시판에 걸쳐 조회하는 페이지 - router/index.js의 /required-posts 경로에 대응
// RequiredController.java의 GET /api/required 엔드포인트를 통해 데이터 수신
export default {
  name: 'RequiredPosts',
  // PageHeader: 페이지 상단 '필독 게시글' 제목 표시
  components: { PageHeader },
  data() {
    return {
      // 게시판별 필독 게시글 목록 (fetchRequiredPosts 호출 후 서버 응답으로 채워짐)
      posts: {
        notice:      [],  // 전체 공지사항 필독 게시글
        teamNotice:  [],  // 팀별 공지사항 필독 게시글
        board:       [],  // 자유 게시판 필독 게시글
        archive:     [],  // 전사 자료실 필독 게시글
        teamArchive: []   // 팀별 자료실 필독 게시글
      },
      // 현재 페이지 번호 (1부터 시작)
      pageNum: 1
    }
  ...
  computed: {
    ...
    // 게시판별 필독 게시글을 하나의 배열로 병합 (팀별 게시판은 "X팀 공지사항" 형식으로 boardLabel 생성)
    flatList() {
      const list = []
    ...
    // 현재 페이지에 표시할 필독 게시글 목록 (10개 단위 슬라이스)
    pagedList() {
  ...
  // 컴포넌트 마운트 시 필독 게시글 목록 로드
  mounted() {
    this.fetchRequiredPosts()
  },
  methods: {
    // GET /api/required?userId={id} 로 필독 게시글 전체 목록 조회 후 게시판별로 분류하여 posts에 저장
    async fetchRequiredPosts() {
      try {
        // localStorage에 저장된 로그인 사용자 ID로 필독 게시글 조회
